        ...
        while self.running:
            ...
            for account_str in config.ACCOUNTS:
                ...
                if len(parts) == 2:
                    ...
                    username, password, nickname = parts[0], parts[1], parts[2]
                else:
                    continue
                ...
                logger.info(f"准备启动机器人: {username} (昵称: {nickname if nickname else '默认'})")
                ...
                if token and entity_id:
                    ...
                    # 创建并启动机器人
                    # 如果配置了昵称，则使用昵称作为游戏内名字，entity_id 用于网易验证
                    bot_name = nickname if nickname else entity_id
                    bot = BotClient(bot_name, token, entity_id, self)
